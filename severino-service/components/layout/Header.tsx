      ...
      >
        <div className="flex h-20 w-full items-center justify-between px-4">
          ...
          <Link href="/" className="ml-1 flex items-center space-x-2 md:ml-5">
            <Image
              src="/Assets/Severino_Logo_1.svg"
              alt="Severino"
              width={752}
              height={752}
          ...
          <div className="flex min-w-0 flex-1 items-center justify-end pr-2 md:pr-6">
            {session?.user ? (
              ...
              <Button
                className="rounded-[4px] border-0 bg-severinostone text-white hover:bg-severinostone/90"
                size="sm"
                type="button"
        ...
        <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
          <SheetContent side="left" className="w-[280px] rounded-r-[4px] sm:w-[320px]">
            <SheetHeader>
              <SheetTitle className="text-xl font-bold text-severinostone">Menu</SheetTitle>
            </SheetHeader>
                ...
                return (
                  <Link
                    ...
                    className={`rounded-[4px] px-4 py-3 text-base font-semibold transition-colors ${
                      isActive ? 'bg-severinogreen text-white' : 'text-black hover:bg-quicksilver'
                    }`}
                  >
                    {item.label}
                  </Link>
