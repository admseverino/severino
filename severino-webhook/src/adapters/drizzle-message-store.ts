...
import { asMsisdn, asPhoneNumberId, asWamid } from '../whatsapp/types.js'
  ...
  async upsertMessages(messages: NormalizedWhatsAppMessage[]): Promise<StoredWhatsAppMessage[]> {
    ...
    const stored = await db()
      .select({
        ...
        phoneNumberId: schema.whatsappMessages.phoneNumberId,
    ...
    return stored.map((row) => ({
      ...
      phoneNumberId: asPhoneNumberId(row.phoneNumberId),
