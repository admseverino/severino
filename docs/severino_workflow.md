...
- **Billing** — a CSV export of period consumption for the external billing system. Paid/unpaid status, invoices, and dunning live entirely in the external system; Severino does not track them.
- **Audit log** — the append-only record of every state transition and value edit.

## Roles & Permissions

Severino defines six roles, scoped from the platform down to a single unit. Higher roles inherit all abilities of lower roles **within their scope**. One user account can hold **any combination of roles across any number of condos** — grants are additive, never mutually exclusive.
...
- Severino **does not track paid/unpaid status, invoices, or amounts due**. Billing lives entirely in the external system; the tenant portal shows **consumption history and reading photos only**.
...
- **Export only**: a CSV download in the format expected by the external billing system → period transitions to `billed`. Severino's billing role ends here.
