  ...
  return (
    <div className="space-y-6" data-testid="meters-grouped-list">
      <section className="space-y-3">
        <h2 className="text-lg font-bold uppercase tracking-wide text-hidrostone">
          Submedidores
        </h2>
        {hasSubmeterContent ? (
          <ul className="space-y-3" data-testid="meters-groups-tree">
            ...
            {ungroupedUnits.length > 0 ? (
              ...
              >
                <h3 className="text-sm font-semibold uppercase tracking-wide text-hidrostone">
                  Sem grupo
                </h3>
        ...
        {masters.length === 0 ? (
          ...
          >
            {masters.map((m) => (
              ...
              >
                <div>
                  <p className="font-semibold text-hidrostone">{masterPrintTitle(m)}</p>
                  {m.meter.identifier ? (
                ...
                <Link
                  ...
                  className="text-sm font-semibold uppercase tracking-wide text-hidrogreen underline-offset-2 hover:underline"
  ...
  return (
    <ul className="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
      {units.map((u) => (
        ...
        >
          <div>
            <p className="font-mono text-sm font-semibold text-hidrostone">{u.unit.label}</p>
          ...
          <div className="flex flex-col items-end gap-1">
            {u.submeters.map((m) => (
              <Link
                key={m.id}
                href={`/reading/${m.id}`}
                className="text-xs font-semibold uppercase tracking-wide text-hidrogreen underline-offset-2 hover:underline"
