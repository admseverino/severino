...
export function getMonorepoRootDir(startDir: string): string {
  ...
  throw new Error(
    'Could not find monorepo root (pnpm-workspace.yaml). Run provisioning from the HidroSync repo.'
  )
}
...
/**
 * Load repo-root `.env` / `.env.local` (no override), then apply `hidrosync-service` `.env`
...
export function loadProvisioningEnv(monorepoRoot: string): void {
  ...
  const serviceDir = path.join(monorepoRoot, 'hidrosync-service')
